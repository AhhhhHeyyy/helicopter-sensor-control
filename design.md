...
- **⚠️ `renderer.autoClear` 陷阱（實機測試踩過一次，2026-09-23）**：全域的 `renderer.autoClear = false`（[index.html:1513](index.html#L1513)）是為了 8th Wall——它自己把相機畫面畫進 WebGL color buffer，若讓 `render()` 自動清除會蓋掉那張畫面，所以原本的做法是在 `render()` 之前手動呼叫 `clear()`/`clearDepth()`。**這個手動 clear 對 WebXR 是錯的**：WebXR session 進行中，`renderer.render()` 內部才會把 framebuffer 換綁到「真正會顯示給使用者看」的 XR framebuffer，在 `render()` 之前手動呼叫的 `clear()` 清的是還沒換綁、沒人看得到的預設 framebuffer——結果就是 3D 物件（直升機、reticle）都正常畫得出來，但背景畫面固定顯示系統預設的不透明黑色，鏡頭透視完全看不到，第一版上線後實機測試就是踩到這個（症狀：「相機是黑的，其他都正常」）。修法：`_startWebXR()` 內把 `renderer.autoClear` 暫時開回 `true`，讓 `render()` 自己在正確換綁後的 framebuffer 上清成透明（`alpha=0`，因為 renderer 建立時有帶 `alpha: true`），animate() 尾端則跳過對 WebXR 的手動 clear；session 結束時要記得把 `autoClear` 改回 `false`，否則會影響之後可能接回的 8th Wall／桌機模式。**教訓**：`autoClear=false` + 手動 clear 的模式，跟「render() 內部會換框架緩衝」的機制（WebXR 正是這樣）互斥，之後如果還要接其他也是內部換框架緩衝的 API，要先檢查這一點。
