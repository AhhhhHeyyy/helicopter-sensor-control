...
- **`window._arBackend`**：新增旗標區分 `'8thwall'` / `'webxr'` / `null`，只用在渲染迴圈最後「清 buffer」的分支（8th Wall 只清深度、保留它自己畫的相機畫面；WebXR 跟桌機一樣要整個清成透明，讓系統合成器疊上真正的鏡頭畫面）。其餘所有「是不是在 AR 模式」的判斷仍沿用原本的 `window._xrActive`（兩種後端都會設為 `true`），不用逐一改判斷式。
